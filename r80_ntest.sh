...
# to run this script
#bash /home/b.mashburn/Hibiscus/3_Hlil/stacks_scripts/r80_ntest.sh
...
mkdir $OUT_DIR/stacks_n$i
# run ustacks with M equal to the current iteration number
id=1
for sample in $FILES
do
	ustacks -f $SAMPLES_DIR/${sample}.fastq.gz -o $OUT_DIR/stacks_n$i -i $id -m 3 -M 1 -p 15
	let "id+=1"
done

## Now run cstacks to compile stacks between samples. Make sure the popmap has been created and updated.
cstacks -n $i -P $OUT_DIR/stacks_n$i -M $HOME_DIR/popmap_all -p 15
## Run sstacks
sstacks -P $OUT_DIR/stacks_n$i -M $HOME_DIR/popmap_all -p 15
## Run tsv2bam to transpose data so it is stored by locus instead of by samples
tsv2bam -P $OUT_DIR/stacks_n$i -M $HOME_DIR/popmap_all -t 15
## Run gstacks
gstacks -P $OUT_DIR/stacks_n$i -M $HOME_DIR/popmap_all -t 15
## Run populations completely unfiltered, output unfiltered VCF to use in RADstackshelpR
populations -P $OUT_DIR/stacks_n$i -M $HOME_DIR/popmap_all --vcf -t 15
done
